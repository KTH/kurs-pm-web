...
async function getContent(req, res, next) {
  try {
    ...
    if (rawMemos.length === 0) {
      ...
      return res.redirect(
        ...redirectToAboutCourseConfig(state, serverConfig.hostUrl, serverConfig.proxyPrefixPath.uri, courseCode)
      )
    }
    ...
    if (!finalMemoEndPoint) {
      const state = createState(id, semester, potentialMemoEndPoint, courseCode)
...
function createState(id, semester, potentialMemoEndPoint, courseCode) {
